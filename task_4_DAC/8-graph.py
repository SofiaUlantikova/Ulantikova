...
with open('settings.txt', 'r') as f:
    ...
    for line in lines:
        print(line)
    data = [float(lines[i].split()[s[i]]) for i in range(len(lines))]
...
plt.text(0.6*timepoints[-1], 0.8*np.max(voltage), f'Время зарядки = {time_charged:4.1f} с', wrap=True)

plt.text(0.6*timepoints[-1], 0.65*np.max(voltage), f'Время разрядки = {time_discharged:4.1f} с', wrap=True)

plt.show()

plt.savefig('/home/b04-304/Ulantikova/task_4_DAC/graphic.svg')